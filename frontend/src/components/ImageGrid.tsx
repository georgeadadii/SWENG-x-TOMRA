...
export const STORE_FEEDBACK = gql`
                ...
                <Tabs defaultValue="batch" value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <div className="border-b border-gray-200">
                        <TabsList className="flex p-0 bg-transparent">
                            <TabsTrigger
                                value="batch"
                                className="px-4 py-2 data-[state=active]:bg-white data-[state=active]:border-b-2 data-[state=active]:border-blue-500 data-[state=active]:shadow-none rounded-none"
                            >
                                Batch View
                            </TabsTrigger>
                            <TabsTrigger
                    ...
                    {/* Batch View - now the default and positioned on the left */}
                    ...
                    {/* Image Grid View - renamed to "View All Images" */}
                    <TabsContent value="grid" className="p-0 m-0" data-testid="content-grid">
                        {loading && (
                        ...
                        {!loading && !error && (
                            <motion.div
                                className="w-full h-[calc(75vh-100px)] overflow-y-auto p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-4 auto-rows-[180px]"
                                variants={container}
                                initial="hidden"
                                animate="show"
                            >
                                {uniqueImages.length === 0 ? (
                                    ...
                                    uniqueImages.map((image, index) => (
                                        <motion.div
                                            key={index}
                                            variants={item}
                                            className={`relative cursor-pointer overflow-hidden rounded-lg ${hoveredIndex === index ? "ring-4 ring-blue-400 ring-opacity-50" : "ring-1 ring-gray-200"
                                                }`}
                                        ...
                                        >
                                            <div className={`absolute inset-0 flex flex-col justify-end p-3 bg-gradient-to-t from-black/80 to-transparent transition-opacity duration-200 ${hoveredIndex === index ? 'opacity-100' : 'opacity-0'
                                                }`}>
                                                ...
                                                <div className="flex items-center mt-1">
                                                    ...
                                                    {annotationStatus[image.imageUrl] && (
                                                        <span className={`ml-auto px-2 py-0.5 text-xs rounded-full ${annotationStatus[image.imageUrl] === "correct"
                            ...
                            <div className="p-6">
                                ...
                                <div className="flex items-center mb-4">
                                    ...
                                    {annotationStatus[selectedImage.imageUrl] && (
                                        <span className={`ml-auto px-2 py-1 text-xs font-medium rounded-full ${annotationStatus[selectedImage.imageUrl] === "correct"
                                ...
                                <div className="flex gap-3">
                                    <button
                                        className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${annotationStatus[selectedImage.imageUrl] === "correct"
                                    ...
                                    <button
                                        className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${annotationStatus[selectedImage.imageUrl] === "incorrect"
