...
// Mock useQuery and useMutation hooks from Apollo Client
...
describe('ImageSwiper Component', () => {
  ...
  const mockStoreFeedback = jest.fn().mockResolvedValue({ data: { storeFeedback: true } });

  beforeEach(() => {
    // Mock the useQuery hook to return mock data
    mockUseQuery.mockReturnValue({ data: mockData, loading: false, error: null });

    // Mock the useMutation hook to return the mock mutation function
    mockUseMutation.mockReturnValue([mockStoreFeedback, { loading: false, error: null }]);
  });
  ...
  test('handles swipe to the right', async () => {
    ...
    // Ensure the mutation was called with the correct arguments
  ...
  test('displays "All images reviewed!" when there are no images', () => {
    ...
    // Check if the "All images reviewed!" message is displayed
